...
# must have a `title`, `description`, `video_file` and `date` key. Additional
# keys are ignored.
#
...
#     echo '{"title": "vmx test video upload", "description": "This is a test from vmx if the upload script works", "video_file": "/tmp/video.mp4", "date": "2024-08-25"}' | ./upload-video.py
#
...
def upload_video(token, title, description, file_path, date):
...
def main(data=None):
    ...
    `description`, `video_file` and `date`.
    """
    if data is None:
        if sys.stdin.isatty():
            print(
                f'Usage: echo \'{{"title": …, "description": …, "video_file": …, "date": …}}\' | upload-video.py'
            )
            return 1
        else:
            data = sys.stdin.read()
    ...
    try:
        title, description, video_file, date = itemgetter(
            "title",
            "description",
            "video_file",
            "date",
        ...
        print(
            "JSON object must contain the keys `title`, `description`, `video_file` and `date`."
        )
        return 4
    ...
    upload_video(token, title, description, video_file, date)
